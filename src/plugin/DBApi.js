//verwion  2021.2.22  增加FixFilter 讓walf不再出錯
//Version  2021.1.28  可拋出錯誤訊息
//Version  2021.1.11  增加JWT認証
//Version  2020.5.11  增加同步呼叫async

import $ from "jquery";

export const DBApi = function(APIUrl, Token, DBKey) {
  ...
  this.Select = function(QueryObj, fn) {
    ...
    QueryObj.Filter = this.FixFilter(QueryObj.Filter);
    QueryObj.Token = Token;
    if (QueryObj.DBKey == undefined) QueryObj.DBKey = this.DBKey;
    var Url = APIUrl + "SelectTable";
    this.AjaxQuery(Url, QueryObj, fn);
  };
  this.SelectTable = function(TableName, Filter, fn, MaxRows = 500) {
  ...
  this.FixFilter = function(Filter) {
    Filter = Filter.replaceAll(")", ")");
    Filter = Filter.replaceAll("(", "(");
    Filter = Filter.replaceAll(" and ", " $$&& ");
    Filter = Filter.replaceAll(" or ", " $$|| ");
    Filter = Filter.replaceAll(" % ", " $$$ ");
    Filter = Filter.replaceAll(" like ", " $$* ");
    Filter = Filter.replaceAll("  ", " ");
    return Filter;
  };
  ...
  this.AjaxQuery = function(url, QueryData, retFunc) {
    return $.ajax({
      ...
      error: function(e, r, h) {
        retFunc({
          isSuccess: false,
          ErrorMsg: " (status:" + e.responseText + " error:" + h + ")",
        });
      },
    });
  };
